            ...
            while True:
                ...
                async for raw in _receive_stream(ws):
                    ...
                    if event == "chat_resolved":
                        ...
                        if chunk_type == "summarize_start":
                            # Notify the user that history compression is
                            # running before the agent responds.
                            console.print("\n[dim]◌  Summarizing chat history…[/dim]")

                        elif chunk_type == "summarize_end":
                            summary_text = msg.get("content", "").strip()
                            if summary_text:
                                console.print(
                                    "[dim]┄┄┄  Chat Summary  ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄[/dim]"
                                )
