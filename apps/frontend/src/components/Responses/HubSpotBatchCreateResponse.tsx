...
export function HubSpotBatchCreateResponse({ results }: { results: HubSpotContact[] }) {
    ...
    return (
        <div className="space-y-4 mt-4 font-generalSans text-text-lm dark:text-text" onClick={(e) => e.stopPropagation()}>
                ...
                {results.map((contact) => (
                    <Sheet key={contact.id}>
                        <SheetTrigger asChild>
                            <div className="group cursor-pointer bg-bg-light-lm dark:bg-bg-light border border-border-lm dark:border-border rounded-xl p-4 hover:shadow-md hover:border-primary-lm dark:hover:border-primary transition-all">
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-bg-lm dark:bg-bg flex items-center justify-center font-bold text-text-muted-lm dark:text-text-muted uppercase">
                                ...
                                <div className="space-y-1.5 border-t border-zinc-50 dark:border-zinc-900 pt-3">
                                    ...
                                    <div className="flex items-center gap-2 text-[11px] text-zinc-500 uppercase font-bold tracking-tighter">
                                        <Badge variant="outline" className="text-[9px] px-1 py-0 border-border-lm dark:border-border">
                        ...
                        <SheetContent side="right" className="w-full sm:max-w-xl p-0 flex flex-col bg-bg-light-lm dark:bg-bg-light border-l border-border-lm dark:border-border font-generalSans">
                            <SheetHeader className="px-6 py-4 border-b border-border-lm dark:border-border bg-bg-lm/50 dark:bg-bg/50">
                            ...
                            <div className="flex-1 overflow-y-auto p-8 space-y-8">
                                <div className="text-center">
                                    ...
                                    <h2 className="text-2xl font-black text-text-lm dark:text-text">
                                        {contact.properties.firstname} {contact.properties.lastname}
                                    </h2>
                                    ...
                                    <div className="p-4 rounded-2xl bg-zinc-50 border border-zinc-100 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-semibold text-text-lm dark:text-text">{contact.properties.email}</span>
                                            </div>
                                            <Badge variant="secondary" className="text-[10px] bg-bg-light-lm dark:bg-bg-light border-border-lm dark:border-border">{contact.properties.hs_email_domain}</Badge>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <Phone className="w-4 h-4 text-zinc-400" />
                                            <span className="text-sm font-semibold text-text-lm dark:text-text">{contact.properties.phone}</span>
                                        </div>
                                    </div>
                            ...
                            <div className="p-4 border-t border-border-lm dark:border-border bg-bg-light-lm dark:bg-bg-light sticky bottom-0">
                                <Button
                                    className="w-full h-12 bg-primary-lm dark:bg-primary hover:bg-primary-lm/90 dark:hover:bg-primary/90 text-white font-bold rounded-xl shadow-lg shadow-primary-lm/20 dark:shadow-none"
