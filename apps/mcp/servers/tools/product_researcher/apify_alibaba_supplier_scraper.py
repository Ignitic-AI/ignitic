"""Alibaba.com supplier discovery via Apify (Alibaba Supplier Scraper actor)."""
...
# Hard cap: never request more than one page (cost control; enforced in code, not user-configurable).
_MAX_PAGES = 1
...
def apify_alibaba_supplier_search(
    ...
    """
    Search Alibaba.com for suppliers/manufacturers via Apify. Returns JSON list of supplier records
    (company names, Gold Supplier tenure, verification flags, response rates, factory clues, profile URLs, etc.).

    Pagination is fixed to a single results page only (max_pages=1); this cannot be increased.

    Args:
        queries: Comma-separated product search phrases (e.g. "wireless earbuds, phone cases").
        min_years_as_gold_supplier: Optional filter — minimum years as Gold Supplier.
        assessed_supplier_only: If True, restrict to assessed suppliers when the actor supports it.
        service_tags: Optional comma-separated tags (e.g. "OEM Service, ODM Service").
        max_results: Max supplier rows to return from the dataset (default 100).

    Returns:
        JSON list of standardized objects with keys: searchQuery, companyId, name, country, countryCode,
        yearsAsGoldSupplier, companyIconUrl, profileUrl, totalEmployees, factorySize, annualRevenue, responseRate,
