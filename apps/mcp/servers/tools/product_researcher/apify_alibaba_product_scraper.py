"""Alibaba.com wholesale product search via Apify (devcake/alibaba-products-scraper)."""
...
def _standardize_item(item: Dict[str, Any]) -> Dict[str, Any]:
    """Stable snake_case rows matching the product-research tool contract."""
...
def apify_alibaba_product_search(
    ...
    """
    Search Alibaba.com for wholesale product listings via Apify. Returns JSON list of standardized
    product rows: search_query, name, price_min, price_max, currency, moq, product_url, main_image,
    company_name, years_as_gold_supplier, supplier_service_score, is_alibaba_guaranteed,
    is_trade_assurance, is_verified_supplier, review_count, review_score, orders_count.

    max_pages is always 1 (hard cap; not configurable).

    Args:
        queries: Comma-separated search phrases (e.g. "wireless earbuds, phone cases").
        filter_moq_min: Optional MOQ floor passed to the actor.
        filter_price_min_usd / filter_price_max_usd: Optional USD price band filters for the search.
        trade_assurance: Only Trade Assurance listings (actor filter).
        verified_supplier: Only verified supplier listings (actor filter).
        alibaba_guaranteed: Only Alibaba Guaranteed listings (actor filter).
        start_page: First results page (1-based); still only one page of results is scraped.
        max_results: Max product rows to return (cap).
    """
